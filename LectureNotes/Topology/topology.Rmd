...
.bg-washed-yellow.br1.shadow-3.ph2.mt3[
**`r icon::fa("question")` Question**

Define function $\langle u,v \rangle = u_{1}v_{1} + 2u_{2}v_{2}$ on $\mathbb{R}^{2}$ where $u = (u_{1},u_{2})$ and $v = (v_{1},v_{2})$.
]
...
Two norms $\|\cdot\|_{a}$ and $\|\cdot\|_{b}$ defined on $(V, \mathbb{F})$ are equivalent if one can be bounded with the other. 

$\exists \alpha, \beta \in \mathbb{R}_{+}$ such that
